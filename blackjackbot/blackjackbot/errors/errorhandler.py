...
async def error_handler(update, context):
    try:
        ...
        logger.error("The update {} raised the following 'Forbidden' exception: {}".format(update, e))
        error_msg = str(e)
        if error_msg == "Forbidden: bot was blocked by the user":
            GameStore().remove_game(update.effective_chat.id)
        elif error_msg == "Forbidden: bot was kicked from the supergroup chat" or error_msg == "Forbidden: bot was kicked from the group chat":
            GameStore().remove_game(update.effective_chat.id)
