...
from utils import display_name, send_async, answer_async, error, TIMEOUT, \
    user_is_creator_or_admin, user_is_creator, game_is_running
...
@user_locale
async def notify_me(update: Update, context: CallbackContext):
    ...
    if update.message.chat.type == 'private':
        ...
        if update.message.chat_id in gm.remind_dict:
            for user in gm.remind_dict[update.message.chat_id]:
                await send_async(context.bot, user,
...
async def leave_game(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if game.started:
            await send_async(context.bot, chat.id,
                             text=__("Okay. Next Player: {name}", multi=game.translate)
                             .format(name=display_name(game.current_player.user)),
                             reply_to_message_id=update.message.message_id)
        else:
            await send_async(context.bot, chat.id,
                             text=__("{name} left the game before it started.", multi=game.translate)
                             .format(name=display_name(user)),
                             reply_to_message_id=update.message.message_id)


@user_locale
async def kick_player(update: Update, context: CallbackContext):
    if update.message.chat.type == 'private':
        await help_handler(update, context)
        return
    chat = update.message.chat
    ...
    if await user_is_creator_or_admin(user, game, context.bot, chat):
        if update.message.reply_to_message:
            ...
            try:
                ...
                gm.end_game(chat, user)
                await send_async(context.bot, chat.id,
                                 text=_("{0} was kicked by {1}"
                                        .format(display_name(kicked), display_name(user))))
                await send_async(context.bot, chat.id,
...
async def select_game(update: Update, context: CallbackContext):
    ...
    for player in players:
        ...
        await send_async(context.bot, update.callback_query.message.chat_id,
...
async def status_update(update: Update, context: CallbackContext):
    ...
    if update.message.left_chat_member:
        ...
        try:
            ...
            await send_async(context.bot, chat.id,
                             text=__("Removing {name} from the game", multi=game.translate)
                             .format(name=display_name(user)))
...
async def start_game(update: Update, context: CallbackContext):
    if update.message.chat.type != 'private':
        ...
        if game.started:
            ...
            choice = [[InlineKeyboardButton(text=_("Make your choice!"),
                                            switch_inline_query_current_chat='')]]
            first_message = (__("First player: {name}\n"
                                "Use /close to stop people from joining the game.\n"
                                "Enable multi-translations with /enable_translations",
                                multi=game.translate)
                             .format(name=display_name(game.current_player.user)))
            await context.bot.send_sticker(chat.id, sticker=c.STICKERS[str(game.last_card)])
            await context.bot.send_message(chat.id, text=first_message,
        ...
        for player in players:
            ...
            groups.append([InlineKeyboardButton(text=title,
...
@user_locale
async def close_game(update: Update, context: CallbackContext):
    chat = update.message.chat
    user = update.message.from_user
    games = gm.chatid_games.get(chat.id)
    if not games:
        await send_async(context.bot, chat.id, text=_("There is no running game in this chat."))
        return
    game = games[-1]
...
@user_locale
async def open_game(update: Update, context: CallbackContext):
    chat = update.message.chat
    user = update.message.from_user
    games = gm.chatid_games.get(chat.id)
    if not games:
...
@user_locale
async def enable_translations(update: Update, context: CallbackContext):
    chat = update.message.chat
    user = update.message.from_user
    games = gm.chatid_games.get(chat.id)
    if not games:
...
@user_locale
async def disable_translations(update: Update, context: CallbackContext):
    chat = update.message.chat
    user = update.message.from_user
    games = gm.chatid_games.get(chat.id)
    if not games:
    ...
    if user.id in game.owner:
        ...
        await send_async(context.bot, chat.id,
                         text=_("Disabled multi-translations. Enable them again with "
                                "/enable_translations"))
    else:
        await send_async(context.bot, chat.id,
                         text=_("Only the game creator ({name}) and admin can do that.")
...
async def skip_player(update: Update, context: CallbackContext):
    ...
    if delta < skipped_player.waiting_time and player != skipped_player:
        ...
        await send_async(context.bot, chat.id,
                         text=_("Please wait {time} second", "Please wait {time} seconds", n)
                         .format(time=n),
                         reply_to_message_id=update.message.message_id)
    else:
...
async def process_result(update: Update, context: CallbackContext):
    ...
    if game_is_running(game):
        nextplayer_message = (__("Next player: {name}", multi=game.translate)
                              .format(name=display_name(game.current_player.user)))
        choice = [[InlineKeyboardButton(text=_("Make your choice!"),
                                        switch_inline_query_current_chat='')]]
        await send_async(context.bot, chat.id, text=nextplayer_message,
                         reply_markup=InlineKeyboardMarkup(choice))
        start_player_countdown(context.bot, game, context.job_queue)
...
def build_app():
    token = os.environ.get('UNO_BOT_TOKEN')
    ...
    app.add_handler(InlineQueryHandler(reply_to_query))
    app.add_handler(ChosenInlineResultHandler(process_result))
    app.add_handler(CallbackQueryHandler(select_game))
    ...
    app.add_handler(CommandHandler('open', open_game))
    app.add_handler(CommandHandler('close', close_game))
    app.add_handler(CommandHandler('enable_translations', enable_translations))
    app.add_handler(CommandHandler('disable_translations', disable_translations))
    app.add_handler(CommandHandler('skip', skip_player))
    app.add_handler(CommandHandler('notify_me', notify_me))
    simple_commands.register(app)
    settings.register(app)
    app.add_handler(MessageHandler(filters.StatusUpdate.ALL, status_update))
